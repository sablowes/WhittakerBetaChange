...
set.seed(101)
local_overall_post <- gather_draws(local_ES_norm, b_Intercept) %>% 
  # 90% credible interval of intercept
  median_qi(.width = 0.9)  

regional_overall_post <- gather_draws(regional_ES_norm, b_Intercept) %>% 
  median_qi(.width = 0.9)
...
local_posterior_ES <- local_ES_norm$data %>% 
  add_predicted_draws(model = local_ES_norm, n = 1000)
...
regional_posterior <- regional_ES_norm$data %>% 
  add_predicted_draws(model = regional_ES_norm, n = 1000)
...
regional_posterior <- left_join(regional_posterior %>% 
                                  rename(regional_dS = .prediction), 
                                all_meta) %>% 
  ungroup()

local_posterior_ES <- left_join(local_posterior_ES %>% 
